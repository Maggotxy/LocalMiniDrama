...
- **项目工作台**：首页增加项目搜索、内容类型/状态筛选、最近更新/完成度/名称排序；根据剧本、分镜、画面、视频片段与成片实时计算完成度和「下一步」提示，并增加「继续制作」快捷入口
- **前端加载性能**：Element Plus 改为组件按需加载，移除全量组件/图标全局注册；AI 配置内容改为异步组件，生产入口包显著缩小
- **可访问性**：增加键盘 `focus-visible` 样式与 `prefers-reduced-motion` 支持

### 安全

- 后端默认仅监听 `127.0.0.1`，CORS 默认仅允许本机来源；局域网开放改为显式配置
- AI 配置 API 不再返回 API Key、Token、Secret Access Key、Kling Secret Key 等敏感字段；已保存配置改为由后端按配置 ID 安全复用凭据
- 移除 `NODE_TLS_REJECT_UNAUTHORIZED=0` 全局 TLS 证书绕过与 TTS 明文 API Key 日志
- 工程 ZIP 导入限制 256MB 上传、5000 个条目、单文件 128MB、解压后 512MB，并升级 `adm-zip` 防止恶意 ZIP 内存放大
- 升级 `sharp`、`multer`、Electron / electron-builder 等关键依赖，并对火山 SDK 的易受影响传递依赖固定安全版本

### 修复

- 素材中心上传图片后同步登记 `assets`，刷新后不再丢失；修复后端关键词搜索、分页总数与文件大小/类型元数据返回
- 发布 CI 增加前后端测试与 production audit，移除安装阶段 TLS 绕过，tag 构建成功后直接发布 Release
